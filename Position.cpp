...
Position::Position(int a,int b):x(a),y(b){}             // constructeur de la classe
...
Position Position::GetNeighborPos(int way)    //cette fonction prendre la direction de mouvement est retourne la position voisin selon cette direction
{
...
bool Position::NeighborCaseValidation(const GameSpace& ech,int way)      //validation si la case voisins est vide ou detruit ou un bord (il utilise la methode GetNeighborPos)
 // ce type de passage par parametre indique que cette methode ne fait pas des changment sur l'echiquier
{
...
bool Position::BlockedCase(const GameSpace& ech)  //tester si les joueurs sont bloquees ou non
{
    for(int i=1 ;i<10;++i)
    {
        if(i!=5)
...
Position Position::FindOptimalNeighborCase(const GameSpace& ech)        //chercher la case optimale pour faire la meilleur movement
{
    ...
    for(int i=1 ;i<10;++i )
    {
        if(i!=5)
        {
            ...
            if(/*1<=i&&i<10&&*/NeighborCaseValidation(ech,i))
            {
                ...
                if(nbFree>=Nbmax)
                {
                    Nbmax=nbFree;                                   //c.a.d il va cherchee la position dont laquelle ele possede le plus grand nombre des cases vides
                    BestWay=i ;
                }
            }
        }
...
int Position::ListOfPlayerLegalMoves(const GameSpace& ech)          //retourner le nombre des cases vides accessible par le joueur
{
    vector<Position> t ;
    for(int i=1;i<=9;i++)
    {
...
vector<Position> Position::ListOfPlayerLegalMoves1(const GameSpace & ech)       //retourner un tableau qui renferme les position des cases vides accessible
{
    vector<Position> t ;
    for(int i=1;i<=9;i++)
    {
